...
for i in range(0,num_len):
    ...
    for j in range(i+1,num_len):
        ...
        #一种的合>0,另一种是合<0，
        #如果小于0，没有继续的必要，可以直接break，那么k就算再往里取，那么也会一直小于0，这样做k会不断的减小，在第一轮直接减为1，并在当前i的循环中，不论j为几，k均为1.所以直接跳过了所有的while，并且还有可能出现j>k的情况，所以错误！！
        
        #如果大于0，就说明当前情况下的加和大于零，当k变小，即取到的值num[k]减小时，是有一定几率取到0的。如果当前的while结束了只有两种可能
        #加和<=0,这种情况下进行相等判断，如果不相等，那就是小于0，也就是说没有符合条件的情况，要么是小于0，要么是大于0
        #k<=j了，此时结果同上
